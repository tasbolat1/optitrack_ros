...
class Take(object):
    ...
    def __init__(self):
        ...
        self.single_markers = dict()      # dict of RigidBody objects, indexed by asset name string
    ...
    def _read_header(self, stream, verbose = False):
        ...
        for col,asset_type,label,ID,field,axis in zip( range(len(self._raw_types)), self._raw_types, self._raw_labels, \
            ...
            if asset_type == "Marker":
                if label in self.single_markers:
                    single_marker = self.single_markers[label]
                else:
                    single_marker = SingleMarker(label, ID)
                    self.single_markers[label] = single_marker

                if field == 'Position':
                    axis_index = {'X':0, 'Y':1, 'Z':2}[axis]
                    setter = single_marker._set_position
                    self._column_map.append(ColumnMapping(setter, axis_index, col))

            elif asset_type == 'Rigid Body':
    ...
    def _read_data(self, stream, verbose = False):
        ...
        for row_num, row in enumerate(stream):
            ...
            for single_marker in self.single_markers.values():
                single_marker._add_frame(frame_t)
